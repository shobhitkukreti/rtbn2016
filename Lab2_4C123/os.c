...
/* Shared Mutex for MailBox */

int tx_mtx=1, rx_mtx=0;
uint32_t shared_data=0;
void OS_MailBox_Init(void){
  // include data field and semaphore
  //***YOU IMPLEMENT THIS FUNCTION*****
	OS_InitSemaphore(&tx_mtx, 1);
	OS_InitSemaphore(&rx_mtx, 0);
...
void OS_MailBox_Send(uint32_t data){
  //***YOU IMPLEMENT THIS FUNCTION*****
	BSP_LCD_OutUDec(9, 0xFFE0); 
	OS_Wait(&tx_mtx);
	BSP_LCD_OutUDec(5, 0xFFE0); 
	shared_data = data;
	OS_Signal(&rx_mtx);
	BSP_LCD_OutUDec(6, 0xFFE0); 
}
...
uint32_t OS_MailBox_Recv(void){ 
	uint32_t data;
  //***YOU IMPLEMENT THIS FUNCTION*****
	BSP_LCD_OutUDec(8, 0xFFE0); 
	OS_Wait(&rx_mtx);
	BSP_LCD_OutUDec(4, 0xFFE0); 
	data = shared_data;
  OS_Signal(&tx_mtx);
	BSP_LCD_OutUDec(7, 0xFFE0); 
